...
export default function SocialLoginButtons({ stacked = false }) {
  ...
  useEffect(() => {
    ...
    fetch(getSocialAuthStatusUrl(), { credentials: 'include' })
      .then(async (response) => {
        ...
        if (!response.ok) {
          throw new Error(data.message || 'Unable to check social login availability.');
        }
      ...
      .catch((error) => {
        ...
        setAuthUrls({});
        setStatusError(error.message || 'Unable to check social login availability.');
      });
  ...
  const handleClick = (providerId, providerName) => {
    if (configuredProviders && !configuredProviders.has(providerId)) {
      toast.info(`${providerName} login is not configured yet. Add OAuth keys to backend/.env`);
      return;
    }
  ...
  return (
    <div className="space-y-sm">
      {statusError ? (
        <p className="text-xs text-error text-center">{statusError}</p>
      ) : null}
      ...
      {!isLoading && !statusError && configuredProviders?.size === 0 && (
        <p className="text-xs text-on-surface-variant text-center">
          Social login needs OAuth keys in <code className="text-secondary">backend/.env</code>. Run{' '}
          <code className="text-secondary">npm run check:social-auth --prefix backend</code> to verify setup.
        </p>
      )}

      {!isLoading && configuredProviders && configuredProviders.size > 0 && configuredProviders.size < providers.length && (
        <p className="text-xs text-on-surface-variant text-center">
          Some providers are not configured on the server yet. Only enabled providers can be used.
        </p>
      )}
    </div>
  );
