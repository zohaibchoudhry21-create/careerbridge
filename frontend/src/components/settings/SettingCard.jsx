...
export default function SettingCard({ title, description, icon, to, color = 'settings' }) {
  ...
  return (
    <button
      type="button"
      onClick={() => navigate(to)}
      className="dashboard-card-hover dashboard-glass-card group flex h-full w-full flex-col items-start rounded-2xl border border-outline-variant/40 bg-white p-sm text-left transition-all duration-200 hover:border-secondary/30 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-secondary/40 sm:p-md"
    >
