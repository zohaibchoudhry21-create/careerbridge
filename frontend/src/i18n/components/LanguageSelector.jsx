    ...
    <div ref={containerRef} className={cn('relative shrink-0', className)}>
      <button
        ...
        className={cn(
          ...
          'font-label-md text-on-surface hover:border-secondary/40 hover:bg-white transition-colors',
          'disabled:opacity-60 min-h-[36px]',
          compact && 'px-2 py-1'
        )}
      >
        <AppIcon name="language" size="h-4 w-4" className="text-on-surface-variant" />
        <span className="tracking-wide">{currentDisplayCode}</span>
        <AppIcon name="expand_more" size="h-4 w-4" className="text-on-surface-variant" />
      </button>
            ...
            return (
              <li key={option.i18nCode} role="option" aria-selected={selected}>
                ...
                >
                  ...
                  <span className="text-xs tracking-wide text-on-surface-variant">{option.display}</span>
                </button>
              </li>
            );
          })}
