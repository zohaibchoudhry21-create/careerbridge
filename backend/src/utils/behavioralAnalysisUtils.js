 ...
 * Keep in sync with frontend/src/features/interviewPrep/utils/behavioralAnalysisUtils.js
 */
...
export const buildBehavioralTimelineEvents = (samples = [], options = {}) => {
  ...
  for (let i = 0; i < samples.length; i += 1) {
    ...
    if (nextHold !== holdType) {
      ...
      nextHold &&
      !holdEmitted &&
      tMs - holdSinceMs >= TIMELINE_EVENT_MIN_HOLD_MS
    ) {
...
/**
 * Expression change timeline (sampled dominant expressions over time).
 */
export const buildExpressionTimeline = (samples = [], options = {}) => {
  ...
  for (let i = 0; i < samples.length; i += 1) {
    ...
    const dominant =
      sample.dominantExpression || getDominantExpression(expressions);
...
/**
 * @param {object} parts measured 0–100 components
 */
...
/**
 * Higher score = more distracted. Uses measured percentages 0–100.
 */
